...
from utils.db import (
    ...
    papers_collection,
    solutions_liked_collection,
    papers_cited_collection,
    papers_liked_collection,
    convert_objectid_to_str,
    solution_eval,
    get_paper_index,
    get_solution_index,
    get_user_index,
    get_async_paper_index,
    get_async_solution_index,
    get_async_user_index,
)
...
async def delete_solution(solution_id):
    if solution_id:
        ...
        if solution:
            delete_result = await solutions_collection.delete_one(
                {"_id": ObjectId(solution_id)}
            )
            cited_result = await papers_cited_collection.delete_many(
...
async def paper_cited(
    papers: List[Dict[str, Any]], solution_ids: List[ObjectId]
) -> None:
...
async def like_solution(user_id: str, solution_id: str):
    ...
    result = await solutions_collection.find_one_and_update(
        {"_id": ObjectId(solution_id)}, {"$inc": {"Liked": 1}}, return_document=True
    )
...
async def test_api_connection(current_user, api_key, api_url=None, model_name=None):
    try:
        ...
        try:
            ...
            LOG.logger.info(
                f"API connection test successful for user {current_user['email']}"
            )
            return {
                "success": True,
            ...
            LOG.logger.error(
                f"API connection test failed for user {current_user['email']}: {error_details}"
            )
            ...
            if "status_code" in error_details and "text" in error_details:
                ...
                return {
                    ...
                    "details": {
                        ...
                        "provider": (
                            "openai" if "openai.com" in str(base_url) else "other"
                        ),
                    },
                }
            else:
                ...
                return {
                    ...
                    "details": {"raw_error": error_details},
                }
